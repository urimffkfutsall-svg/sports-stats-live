...
const LiveMatchPage = () => {
  ...
  const LiveCard = ({ match }) => {
    ...
    return (
      <div onClick={() => setSelectedMatch(match)} className="relative bg-white rounded-2xl overflow-hidden cursor-pointer group hover:shadow-xl transition-all duration-300 border-2 border-[#0f1830]/30 shadow-lg shadow-[#0f1830]/15">
        <div className="bg-gradient-to-r from-[#0f1830] to-[#3B82F6] px-5 py-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center justify-between px-6 py-6">
          ...
          <div className="flex flex-col items-center gap-2 px-4">
            <div className="flex items-center gap-2">
              <span className="text-5xl font-black text-[#0f1830] tabular-nums">{match.homeScore ?? 0}</span>
              <div className="flex flex-col items-center"><span className="w-1.5 h-1.5 bg-[#0f1830] rounded-full"></span><span className="w-0.5 h-3 bg-gray-200"></span><span className="w-1.5 h-1.5 bg-[#0f1830] rounded-full"></span></div>
              <span className="text-5xl font-black text-[#0f1830] tabular-nums">{match.awayScore ?? 0}</span>
            </div>
        ...
        <div className="px-5 pb-4"><div className="w-full flex items-center justify-center gap-1.5 py-2 bg-[#0f1830]/5 hover:bg-[#0f1830]/10 text-[#0f1830] text-[11px] font-bold rounded-lg transition-colors border border-[#0f1830]/10">SHIKO DETAJET</div></div>
      </div>
    );
  };
  ...
  const SmallCard = ({ match, type }) => {
    ...
    const away = getTeamById(match.awayTeamId);
    return (
      <div onClick={() => setSelectedMatch(match)} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md hover:border-[#0f1830]/20 transition-all cursor-pointer">
        <div className="px-3 py-2.5 flex items-center justify-between">
          ...
          <div className="px-2 min-w-[50px] text-center">
            {type === 'finished' ? (
              ...
              <span className="text-xs font-bold text-[#0f1830]">{match.time || 'VS'}</span>
            )}
          </div>
    ...
    return (
      <div className="mb-8">
        <h2 className="text-lg font-black text-gray-900 mb-4 flex items-center gap-2">
          <span className="w-1 h-6 bg-[#0f1830] rounded-full"></span>{name}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {upcoming.length > 0 && (<div><h3 className="text-sm font-bold text-gray-500 mb-3 flex items-center gap-2"><span className="w-2 h-2 bg-[#0f1830] rounded-full"></span>Ndeshjet e Ardhshme</h3><div className="space-y-2">{upcoming.map(m => <SmallCard key={m.id} match={m} type="planned" />)}</div></div>)}
        </div>
      </div>
    );
  };
    ...
    <div className="min-h-screen bg-[#F1F5F9] flex flex-col">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#0f1830]/10 rounded-full mb-3">
            <span className="relative flex h-2.5 w-2.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#0f1830] opacity-50"></span><span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#0f1830]"></span></span>
            <span className="text-[#0f1830] text-xs font-black uppercase tracking-[0.15em]">Live</span>
          </div>
        ...
        {liveMatches.length > 0 ? (
          ...
          <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 text-center py-14 mb-10">
            <div className="w-16 h-16 bg-[#0f1830]/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-[#0f1830]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
            </div>
