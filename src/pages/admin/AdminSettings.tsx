          ...
          {showSeasonForm && (
            <form onSubmit={handleAddSeason} className="bg-white rounded-xl border border-gray-200 p-4 mb-4 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => { setShowSeasonForm(false); setEditSeasonId(null); }} className="text-gray-400 hover:text-gray-600">✕</button>
              </div>
            </form>
          )}
              ...
              seasons.map(s => (
                <div key={s.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => { setSeasonForm({ name: s.name, startDate: s.startDate, endDate: s.endDate, isActive: s.isActive }); setEditSeasonId(s.id); setShowSeasonForm(true); }} className="p-1.5 text-gray-400 hover:text-[#1E6FF2]">✎</button>
                    <button onClick={() => { if (confirm('Fshi sezonin?')) deleteSeason(s.id); }} className="p-1.5 text-gray-400 hover:text-red-500">✗</button>
                  </div>
                </div>
              ))
            )}
          ...
          {showCompForm && (
            <form onSubmit={handleAddComp} className="bg-white rounded-xl border border-gray-200 p-4 mb-4 grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowCompForm(false)} className="text-gray-400">✕</button>
              </div>
            </form>
          )}
          <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
                ...
                return (
                  <div key={c.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => { if (confirm('Fshi kompeticionin?')) deleteCompetition(c.id); }} className="p-1.5 text-gray-400 hover:text-red-500">✗</button>
                    </div>
                  </div>
                );
              })
        ...
        <div>
          ...
          {showEditorForm && (
            <form onSubmit={handleAddEditor} className="bg-white rounded-xl border border-gray-200 p-4 mb-4 grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
              ...
              <div className="flex gap-2">
                <button type="submit" className="px-4 py-2 bg-[#1E6FF2] text-white rounded-lg text-sm hover:bg-[#1558CC]">Shto</button>
                <button type="button" onClick={() => setShowEditorForm(false)} className="text-gray-400">✕</button>
              </div>
            </form>
          )}
          <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
            {users.filter(u => u.role === 'editor').length === 0 ? (
              ...
              users.filter(u => u.role === 'editor').map(u => (
                <div key={u.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50">
                  ...
                  <button onClick={() => { if (confirm('Fshi editorin?')) deleteUser(u.id); }} className="p-1.5 text-gray-400 hover:text-red-500">✗</button>
                </div>
              ))
            )}
          </div>
