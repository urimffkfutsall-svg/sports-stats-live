...
// =====================================================
// STEP 1: Rewrite LiveMatchPage with white/blue design
// =====================================================
...
const LiveMatchPage: React.FC = () => {
  ...
  const getRecent = (compId?: string) => {
    if (!compId) return [];
    return seasonMatches
      .filter(m => m.competitionId === compId && m.status === 'finished')
      .sort((a, b) => (b.date || '').localeCompare(a.date || ''))
      .slice(0, 4);
  };

  const getUpcoming = (compId?: string) => {
    if (!compId) return [];
    return seasonMatches
      .filter(m => m.competitionId === compId && m.status === 'planned')
      .sort((a, b) => (a.date || '').localeCompare(b.date || ''))
      .slice(0, 4);
  };

  // ============ LIVE CARD (White + Blue) ============
  const LiveCard: React.FC<{ match: Match }> = ({ match }) => {
    ...
    return (
      <div
        ...
        className="relative bg-white rounded-2xl overflow-hidden cursor-pointer group hover:shadow-xl transition-all duration-300 border-2 border-[#1E6FF2]/30 shadow-lg shadow-blue-100"
      >
        {/* Top bar */}
        ...
        <div className="flex items-center justify-between px-6 py-6">
          ...
          <div className="flex flex-col items-center gap-3 flex-1">
            <div className="w-[72px] h-[72px] rounded-2xl bg-gradient-to-b from-gray-50 to-white border-2 border-gray-100 flex items-center justify-center overflow-hidden p-2 group-hover:border-[#1E6FF2]/20 transition-colors">
              {home?.logo ? (
                <img src={home.logo} alt="" className="w-full h-full object-contain" />
              ) : (
                <span className="text-gray-300 text-2xl font-black">{home?.name?.charAt(0)}</span>
              )}
            </div>
          ...
          <div className="flex flex-col items-center gap-2 px-4">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col items-center">
                <span className="w-1.5 h-1.5 bg-[#1E6FF2] rounded-full"></span>
          ...
          <div className="flex flex-col items-center gap-3 flex-1">
            <div className="w-[72px] h-[72px] rounded-2xl bg-gradient-to-b from-gray-50 to-white border-2 border-gray-100 flex items-center justify-center overflow-hidden p-2 group-hover:border-[#1E6FF2]/20 transition-colors">
              {away?.logo ? (
                ...
                <span className="text-gray-300 text-2xl font-black">{away?.name?.charAt(0)}</span>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  {homeGoals.map((g, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-1.5 text-gray-600 text-xs">
                        ...
                        <span className="font-medium">{p ? p.firstName.charAt(0) + '. ' + p.lastName : '?'}</span>
                        <span className="text-gray-300 text-[10px]">{g.minute}'</span>
                      </div>
                    );
                  })}
                </div>
                <div className="space-y-1.5 text-right">
                  {awayGoals.map((g, i) => {
                    const p = players.find(pl => pl.id === g.playerId);
                    return (
                      <div key={i} className="flex items-center gap-1.5 text-gray-600 text-xs justify-end">
                        <span className="text-gray-300 text-[10px]">{g.minute}'</span>
                        <span className="font-medium">{p ? p.firstName.charAt(0) + '. ' + p.lastName : '?'}</span>
                        <span className="text-[10px]">\\u26BD</span>
                      </div>
        ...
        {/* Shiko detajet */}
        <div className="px-5 pb-4">
          <div className="w-full flex items-center justify-center gap-1.5 py-2 bg-[#1E6FF2]/5 hover:bg-[#1E6FF2]/10 text-[#1E6FF2] text-[11px] font-bold rounded-lg transition-colors border border-[#1E6FF2]/10">
            SHIKO DETAJET
          </div>
        </div>
      </div>
  ...
  const SmallCard: React.FC<{ match: Match; type: 'finished' | 'planned' }> = ({ match, type }) => {
    ...
    return (
      <div
        onClick={() => setSelectedMatch(match)}
        className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md hover:border-[#1E6FF2]/20 transition-all cursor-pointer"
      >
  ...
  // ============ COMPETITION SECTION ============
  const CompSection: React.FC<{ name: string; compId?: string }> = ({ name, compId }) => {
    ...
    return (
      <div className="mb-8">
        <h2 className="text-lg font-black text-gray-900 mb-4 flex items-center gap-2">
          <span className="w-1 h-6 bg-[#1E6FF2] rounded-full"></span>
          {name}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {recent.length > 0 && (
            <div>
              ...
              <div className="space-y-2">{recent.map(m => <SmallCard key={m.id} match={m} type="finished" />)}</div>
            </div>
          )}
          {upcoming.length > 0 && (
            <div>
              <h3 className="text-sm font-bold text-gray-500 mb-3 flex items-center gap-2">
                <span className="w-2 h-2 bg-[#1E6FF2] rounded-full"></span>
                Ndeshjet e Ardhshme
              </h3>
              <div className="space-y-2">{upcoming.map(m => <SmallCard key={m.id} match={m} type="planned" />)}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#1E6FF2]/10 rounded-full mb-3">
            <span className="relative flex h-2.5 w-2.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#1E6FF2] opacity-50"></span>
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#1E6FF2]"></span>
            </span>
...
console.log("[OK] LiveMatchPage - white/blue design with all competitions");
console.log("Lines: " + liveMatchPage.split("\n").length);
