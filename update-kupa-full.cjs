          ...
          rounds.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <div className="flex gap-8 min-w-max pb-4">
                {rounds.map(r => (
                  <div key={r.round} className="min-w-[400px]">
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2 flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      <span className={\`text-sm font-bold truncate block \${tie.winner === tie.teamA ? 'text-[#1E6FF2]' : 'text-gray-700'}\`}>{teamA?.name || 'TBD'}</span>
                                      <span className="text-[10px] text-gray-400">{tie.winsA} fitore</span>
                                    </div>
                                  </div>
                                  <div className="flex flex-col items-center mx-3">
                                    ...
                                    <span className="text-[9px] text-gray-400 font-semibold uppercase">Agregati</span>
                                  </div>
                                  <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
                                    <div className="min-w-0 text-right">
                                      <span className={\`text-sm font-bold truncate block \${tie.winner === tie.teamB ? 'text-[#1E6FF2]' : 'text-gray-700'}\`}>{teamB?.name || 'TBD'}</span>
                                      <span className="text-[10px] text-gray-400">{tie.winsB} fitore</span>
                                    </div>
                                    <div className="w-7 h-7 rounded-lg bg-white border border-gray-200 overflow-hidden flex-shrink-0">
...
console.log("[OK] KupaPage.tsx — Best of 3 + aggregate + winner detection");

// =====================================================
// ADMIN: Allow all teams for cup final
// =====================================================
let admin = fs.readFileSync("src/pages/admin/AdminMatches.tsx", "utf8");

// Find where teams are filtered for the form and add option to show all teams for cup
// Look for the compTeams or teams filter
const hasCompTeams = admin.includes("compTeams");

if (!admin.includes("showAllTeamsForCupFinal")) {
  // Add after selectedIsCup
  admin = admin.replace(
    "const selectedIsCup = isCupComp(form.competitionId);",
    `const selectedIsCup = isCupComp(form.competitionId);
  const isCupFinal = selectedIsCup && form.week === 4;`
  );

  // Find team select options - show all teams for cup
  // The team selects filter by compTeams or teams - need to see
  // For cup matches, show ALL teams (from all competitions)
  if (admin.includes("compTeams")) {
    admin = admin.replace(
      /compTeams\.filter/g,
      "(selectedIsCup ? teams : compTeams).filter"
    );
    admin = admin.replace(
      /compTeams\.map/g,
      "(selectedIsCup ? teams : compTeams).map"
    );
  }
}

fs.writeFileSync("src/pages/admin/AdminMatches.tsx", admin, "utf8");
console.log("[OK] AdminMatches.tsx — Cup shows all teams from all leagues");
